...
- [x] Prestatiescore naar >90 — _**92** op de CI-runner, gemeten 23-09-2026 (mediaan van drie; FCP 1,4 s, LCP 3,0 s). Drempel in `scripts/lighthouse-check.mjs` staat op 88_
  ...
  - **De marge is klein.** Drie rondes op de runner schommelen een paar punten. Wat de LCP nog op 3,0 s houdt is de HTML met alle ingelijnde CSS (25 KB ingepakt), React en de andere vendorchunks en het lettertype (49 KB). De volgende stappen, als het nodig is: de CSS van het inlogscherm apart, of axios vervangen door `fetch`
  - Eerder, 24-08-2026: **84** op de CI-runner, tegen **79** op `main` daarvoor
  - **Het cijfer hangt af van de machine, en dat is hier de kern.** Dezelfde build haalt lokaal een paar punten meer dan op de CI-runner (deze ronde 94-95 tegen 92; de vorige 91 tegen 84). De CI-runner is de maat, want daar gaat de drempel af
