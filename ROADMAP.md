...
- [ ] Prestatiescore naar >90 — _**84** op de CI-runner, gemeten 24-08-2026. Dezelfde runner mat op `main` **79**. Drempel in `scripts/lighthouse-check.mjs` staat op 80_
  ...
  - Wat de verbetering werkelijk is, beide vandaag op dezelfde soort runner gemeten: **79 → 84** voor performance, **96 → 100** voor best-practices. Drie runs van vrijwel dezelfde build gaven 84, 86 en 84 — reken op 85 ± 1
  - **Waarom >90 hier niet met bijschaven te halen is.** De metrieken wijzen één kant op: TBT 10-20 ms en CLS 0,001 staan op 100 met ruimte over, alle verloren punten zitten in FCP (54) en LCP (66). LCP staat op 3,4 s; voor >90 moet dat naar ongeveer 2,5 s. Het enige overgebleven brok op het kritieke pad is het Nederlandse vertaalbestand, ongeveer 55 KB van 236 KB (ingepakt). Zelfs als dat helemaal weg zou kunnen scheelt dat in de orde van 0,3 s, niet 0,9 s. De rest is React, de router, de queryclient, i18next en het schild zelf
  - Wat wél zou werken is structureel: het inlogscherm los van de applicatiebundel serveren, of de eerste weergave aan de serverkant maken. Dat is een verbouwing, geen instelling, en hij hoort als eigen werkpakket ingepland te worden in plaats van hier te blijven hangen
