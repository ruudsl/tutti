...
function renderMarkdown(content: string): string {
  return (
    content
      ...
      .replace(
        /^> (.+)$/gm,
        '<blockquote class="border-l-4 border-primary pl-4 py-1 my-2 italic text-base-content/80">$1</blockquote>',
      )
      // Checkboxes
      .replace(
        /^- \[x\] (.+)$/gm,
        '<div class="flex items-center gap-2 my-1"><input type="checkbox" class="checkbox checkbox-sm" checked disabled /><span>$1</span></div>',
      )
      .replace(
        /^- \[ \] (.+)$/gm,
        '<div class="flex items-center gap-2 my-1"><input type="checkbox" class="checkbox checkbox-sm" disabled /><span>$1</span></div>',
      )
      // Unordered lists
      ...
      .replace(/^\|(.+)\|$/gm, (_match, content) => {
        ...
        return (
          '<tr>' +
          cells.map((c: string) => `<${cellTag} class="border border-base-300 px-3 py-2">${c}</${cellTag}>`).join('') +
          '</tr>'
        );
      })
