    ...
    <div className="association-switcher" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="association-switcher-dropdown">
          <div className="dropdown-header">{t('multiAssociation.yourAssociations')}</div>
            ...
            {associations.map((assoc) => (
              ...
              >
                <div className="association-info">
                  <span className="association-display-name">{assoc.displayName || assoc.name}</span>
                  {assoc.myRole && (
                    <span className="association-role">{t(`multiAssociation.roles.${assoc.myRole}`)}</span>
                  )}
                </div>
                {assoc.id === user?.associationId && <Icon name="check" size={16} className="check-icon" />}
              </button>
            ))}
          </div>
        </div>
