...
export default function Events() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return statusOption ? (
      <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusOption.color}`}>{statusOption.label}</span>
    ) : null;
  };
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border">
              ...
              <div>
                ...
                <div className="text-gray-600 space-y-1">
                  <p className="flex items-center gap-2">
                    ...
                    {eventDetail.performanceTime &&
                      ` - ${new Date(eventDetail.performanceTime).toLocaleTimeString(currentLocale(), { hour: '2-digit', minute: '2-digit' })}`}
                  </p>
          ...
          <div className="p-6">
            ...
            {activeTab === 'weather' && <EventWeatherTab event={eventDetail} weather={weather} />}
          </div>
        </div>
      </div>
    );
                ...
                <li key={point.id} className="flex items-start gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <div className="text-gray-500">
                      {new Date(point.meetingTime).toLocaleTimeString(currentLocale(), {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="bg-green-50 rounded-lg p-3">
                <div className="text-2xl font-bold text-green-600">{attendanceSummary.byStatus?.attending || 0}</div>
                <div className="text-sm text-gray-600">Aanwezig</div>
              </div>
              <div className="bg-red-50 rounded-lg p-3">
                <div className="text-2xl font-bold text-red-600">{attendanceSummary.byStatus?.not_attending || 0}</div>
                <div className="text-sm text-gray-600">Afwezig</div>
              </div>
              <div className="bg-yellow-50 rounded-lg p-3">
                <div className="text-2xl font-bold text-yellow-600">{attendanceSummary.byStatus?.maybe || 0}</div>
            ...
            <div className="max-h-64 overflow-y-auto space-y-2">
              {event.attendance.map((att) => (
                <div key={att.id} className="flex items-center justify-between text-sm py-1 border-b">
                  ...
                  >
                    {att.status === 'attending' ? 'Aanwezig' : att.status === 'not_attending' ? 'Afwezig' : 'Misschien'}
                  </span>
                </div>
              ))}
            </div>
    ...
    <div>
      ...
      {showAddForm && (
        <div className="mb-4 p-4 border rounded-lg bg-gray-50">
          ...
          <div className="flex gap-2">
            <button onClick={handleAdd} className="px-3 py-1.5 bg-green-600 text-white text-sm rounded-lg">
              Opslaan
            </button>
            ...
            <div key={item.id} className="flex items-center justify-between p-3 border rounded-lg">
              <div className="flex items-center gap-4">
                <div className="text-sm font-medium w-16">
                  {new Date(item.startTime).toLocaleTimeString(currentLocale(), { hour: '2-digit', minute: '2-digit' })}
                </div>
                <div>
                  ...
                  <div>
                    ...
                    {t.departureTime && (
                      <div className="text-sm text-gray-600">
                        Vertrek:{' '}
                        {new Date(t.departureTime).toLocaleTimeString(currentLocale(), {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
          ...
          {packingLists.map((list) => (
            <div key={list.id} className="border rounded-lg p-4">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div className="bg-blue-600 h-2 rounded-full transition-all" style={{ width: `${list.progress}%` }} />
              </div>
            </div>
          ))}
        </div>
        ...
        {weather.forecasts.map((forecast: any) => (
          <div key={forecast.id} className="border rounded-lg p-4">
            <div className="text-sm text-gray-500 mb-2">
              {new Date(forecast.forecastDate).toLocaleDateString(currentLocale(), {
                weekday: 'long',
                day: 'numeric',
                month: 'long',
              })}
            ...
            <div className="flex items-center gap-4">
              <div className="text-3xl font-bold">{forecast.temperatureC}°</div>
              <div className="text-sm text-gray-600">{forecast.weatherDescription}</div>
            </div>
