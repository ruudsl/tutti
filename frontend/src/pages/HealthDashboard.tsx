...
export default function HealthDashboard() {
  ...
  if (error) {
    const isPermissionError =
      axios.isAxiosError(error) && (error.response?.status === 403 || error.response?.status === 401);
    return (
    ...
    <div className="service-card">
      <div className="service-card-header">
        <span className="service-card-title">
          <span className="status-indicator" style={{ backgroundColor: getStatusColor(service.status) }}></span>
          {name}
        </span>
        <span className={`badge ${getStatusBadgeClass(service.status)}`}>{t(`health.status.${service.status}`)}</span>
      </div>
      {service.message && <p style={{ color: 'var(--warning)', marginBottom: '0.5rem' }}>{service.message}</p>}
