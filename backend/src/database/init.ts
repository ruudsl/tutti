...
async function initializeDatabase() {
  ...
  if (existingInstruments.count === 0) {
    ...
    const insertInstrument = db.prepare('INSERT INTO instruments (id, name, tuning) VALUES (?, ?, ?)');
  ...
  const adminUser = db.prepare('SELECT id FROM users WHERE email = ?').get('admin@harmonie.nl') as
    { id: string } | undefined;
  if (adminUser) {
  ...
  try {
    ...
    if (!hasOrchestraId) {
      ...
      db.prepare(
        'ALTER TABLE rehearsals ADD COLUMN orchestra_id TEXT REFERENCES orchestras(id) ON DELETE SET NULL',
      ).run();
      db.prepare(
        'ALTER TABLE rehearsal_default_days ADD COLUMN orchestra_id TEXT REFERENCES orchestras(id) ON DELETE SET NULL',
      ).run();
      console.log('Migration complete');
    }
  } catch (e) {
  ...
  try {
    const addAliasIfNotExists = (instrumentName: string, tuning: string | null, alias: string) => {
      ...
      const existing = db
        .prepare('SELECT id FROM instrument_aliases WHERE instrument_id = ? AND alias = ?')
  ...
  try {
    const tables = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='spond_orchestra_groups'")
      .get();
    if (!tables) {
  ...
  try {
    const tables = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='seating_notification_settings'")
      .get();
    if (!tables) {
  ...
  try {
    const tables = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='seating_notification_logs'")
      .get();
    if (!tables) {
  ...
  try {
    ...
    if (!hasNotificationType) {
      ...
      db.prepare(
        "ALTER TABLE seating_notification_settings ADD COLUMN notification_type TEXT NOT NULL DEFAULT 'webhook'",
      ).run();
  ...
  try {
    const tables = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='instrument_job_title_mappings'")
      .get();
    if (!tables) {
  ...
  try {
    ...
    for (const association of associations) {
      ...
      const existingCount = db
        .prepare('SELECT COUNT(*) as count FROM instrument_job_title_mappings WHERE association_id = ?')
        ...
        for (const mapping of defaultJobTitleMappings) {
          ...
          if (instrument) {
            ...
            const existingMapping = db
              .prepare('SELECT id FROM instrument_job_title_mappings WHERE association_id = ? AND instrument_id = ?')
  ...
  try {
    const tables = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='user_calendar_settings'")
      .get();
    if (!tables) {
