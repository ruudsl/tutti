...
const createFieldDefinitionSchema = z.object({
  ...
  fieldKey: z
    ...
    .regex(
      /^[a-z][a-z0-9_]*$/,
      'Field key moet beginnen met een letter en mag alleen kleine letters, cijfers en underscores bevatten.',
    ),
...
router.patch(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (data.fieldOptions !== undefined) {
      ...
      values.push(data.fieldOptions ? JSON.stringify(data.fieldOptions) : null);
    }
    ...
    if (data.isUnique !== undefined) {
      ...
      values.push(data.isUnique ? 1 : 0);
    }
    if (data.visibility !== undefined) {
      updates.push('visibility = ?');
      values.push(data.visibility);
    }
    if (data.selfEditable !== undefined) {
      updates.push('self_editable = ?');
      values.push(data.selfEditable ? 1 : 0);
    }
    ...
    if (data.validationRegex !== undefined) {
      updates.push('validation_regex = ?');
      values.push(data.validationRegex || null);
    }
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    db.prepare('UPDATE custom_field_definitions SET deleted_at = CURRENT_TIMESTAMP WHERE id = ?').run(req.params.id);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    for (const def of definitions) {
      ...
      if (
        def.visibility === 'committee_plus' &&
        !['admin', 'music_committee', 'equipment_committee', 'uniforms_committee', 'conductor'].includes(userRole)
      )
        canView = false;
...
router.delete(
  '/values/:entityType/:entityId/:fieldKey',
  authenticateToken,
  requireRole('admin'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const associationId = req.user!.associationId;
