...
function generateM365Password(): string {
  ...
  const remaining = Array.from({ length: 8 }, () => allChars[crypto.randomInt(allChars.length)]);
...
async function assignM365License(accessToken: string, userId: string): Promise<boolean> {
  try {
    ...
    const skuData = (await skuResponse.json()) as {
      value: { skuId: string; skuPartNumber: string; consumedUnits: number; prepaidUnits: { enabled: number } }[];
    };
    ...
    let selectedSku:
      { skuId: string; skuPartNumber: string; consumedUnits: number; prepaidUnits: { enabled: number } } | undefined;
 ...
 */
async function addUserToM365Groups(
  accessToken: string,
  userId: string,
  groupNames: string[],
...
function getJobTitleForInstrument(instrumentId: string, associationId: string): string | null {
  ...
  const instrument = db.prepare('SELECT name FROM instruments WHERE id = ?').get(instrumentId) as
...
function isPercussionInstrument(instrumentId: string): boolean {
  ...
  return percussionNames.some((name) => instrument.name.toLowerCase().includes(name.toLowerCase()));
}
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
      ...
      if (msConfig) {
        try {
          ...
          const orgData = (await orgResponse.json()) as {
            value: { verifiedDomains: { name: string; isDefault: boolean }[] }[];
          };
          ...
          const upn =
            email.includes('@') && email.endsWith(defaultDomain)
              ? email
          ...
          if (orchestraIds.length > 0) {
            const orchestraNames = orchestraIds
              .map((orchId: string) => {
                const orch = db.prepare('SELECT name FROM orchestras WHERE id = ?').get(orchId) as
                  { name: string } | undefined;
                return orch?.name;
              })
              .filter(Boolean);
        ...
        if (emailForwardingSet) {
          db.prepare(
            `
                ...
                `,
          ).run(uuidv4(), userId, req.user!.associationId, JSON.stringify({ privateEmail, method: 'initial' }));
        } else {
    ...
    if (m365Created) {
      ...
      if (licenseAssigned) {
        ...
        m365Status +=
          ' ⚠️ WAARSCHUWING: Licentie kon NIET worden toegewezen. Controleer of er beschikbare licenties zijn in het Microsoft 365 admin center.';
      }
      ...
      if (jobTitleSet) {
        ...
        m365Status +=
          ' ⚠️ Functietitel niet ingesteld (geen mapping gevonden voor instrument). Configureer dit in Instellingen → M365 Integratie.';
      }
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare('DELETE FROM pending_spond_links WHERE id = ? AND association_id = ?')
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `,
      ...
      .get(userId, req.user!.associationId) as
      | {
          id: string;
          email: string;
          first_name: string;
    ...
    if (
      !msConfig ||
      !msConfig.microsoft_enabled ||
      !msConfig.microsoft_client_id ||
      !msConfig.microsoft_client_secret ||
      !msConfig.microsoft_tenant_id
    ) {
    ...
    if (success) {
      ...
      db.prepare(
        ...
        `,
      ).run(uuidv4(), userId, req.user!.associationId, JSON.stringify({ privateEmail: user.private_email }));
      ...
      throw new ApiError(
        500,
        `Email forwarding kon niet worden ingesteld. De Exchange mailbox van Microsoft 365 is waarschijnlijk nog niet volledig ingericht (dit kan tot 30 minuten duren na het aanmaken van een nieuw account).${retryInfo} Het systeem probeert automatisch opnieuw, of u kunt later handmatig opnieuw proberen.`,
      );
    }
  }),
);
      ...
      if (msConfig) {
        try {
          ...
          if (deleteResponse.ok || deleteResponse.status === 404) {
            m365Removed = true;
            logger.info(`M365 user deleted: ${user.email}`, {
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (orchestraId) {
      const existing = db
        .prepare('SELECT id FROM m365_group_mappings WHERE orchestra_id = ? AND association_id = ?')
    ...
    if (groupType === 'percussion') {
      const existing = db
        .prepare("SELECT id FROM m365_group_mappings WHERE group_type = 'percussion' AND association_id = ?")
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare('DELETE FROM m365_group_mappings WHERE id = ? AND association_id = ?')
      .run(req.params.id, req.user!.associationId);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id FROM instrument_job_title_mappings WHERE instrument_id = ? AND association_id = ?')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare('DELETE FROM instrument_job_title_mappings WHERE id = ? AND association_id = ?')
      .run(req.params.id, req.user!.associationId);
