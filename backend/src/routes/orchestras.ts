...
router.get(
  '/',
  authenticateToken,
  cacheMiddleware({ ttlSeconds: 300, varyByAssociation: true }),
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id FROM orchestras WHERE LOWER(name) = LOWER(?) AND association_id = ? AND id != ?')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const orchestraToDelete = db
      .prepare('SELECT name FROM orchestras WHERE id = ? AND association_id = ?')
    ...
    const result = db
      .prepare('DELETE FROM orchestras WHERE id = ? AND association_id = ?')
