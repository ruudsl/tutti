 ...
 *       - bearerAuth: []
 */
router.post('/:id/milestones', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    if (!['admin', 'conductor', 'music_committee'].includes(req.user!.role)) {
 ...
 *       - bearerAuth: []
 */
router.patch('/milestones/:id', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    if (!['admin', 'conductor', 'music_committee'].includes(req.user!.role)) {
 ...
 *       - bearerAuth: []
 */
router.delete('/milestones/:id', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    if (!['admin', 'conductor', 'music_committee'].includes(req.user!.role)) {
 ...
 *       - bearerAuth: []
 */
router.post('/milestones/:id/section-progress', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const canUpdate = ['admin', 'conductor', 'music_committee'].includes(req.user!.role) ||
        db.prepare(`SELECT 1 FROM user_instruments WHERE user_id = ? AND instrument_id = ?`).get(req.user!.id, instrumentId);
 ...
 *       - bearerAuth: []
 */
router.post('/:id/initialize-sections', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    if (!['admin', 'conductor', 'music_committee'].includes(req.user!.role)) {
