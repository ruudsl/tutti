 ...
 *     tags: [Calendar]
 */
router.get('/export/:type/:id', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *     tags: [Calendar]
 */
router.get('/feed/:userId', asyncHandler(async (req, res: Response) => {
    ...
    const userRecord = db.prepare('SELECT association_id FROM users WHERE id = ?').get(userId) as { association_id: string } | undefined;
    if (!userRecord) {
        throw new ApiError(404, 'Gebruiker niet gevonden.');
    }
...
router.put('/settings', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (includeR !== undefined) {
        db.prepare('UPDATE user_calendar_settings SET include_rehearsals = ? WHERE user_id = ?')
            .run(includeR, req.user!.id);
    }
    if (includeC !== undefined) {
        db.prepare('UPDATE user_calendar_settings SET include_concerts = ? WHERE user_id = ?')
            .run(includeC, req.user!.id);
    }
    if (googleCalendarId !== undefined) {
        db.prepare('UPDATE user_calendar_settings SET google_calendar_id = ? WHERE user_id = ?')
            .run(googleCalendarId, req.user!.id);
    }
 ...
 *     tags: [Calendar]
 */
router.post('/feed/regenerate', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *     tags: [Calendar]
 */
router.post('/google/auth', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `).get(req.user!.associationId) as { google_calendar_client_id: string; google_calendar_client_secret: string } | undefined;
...
router.get('/google/callback', asyncHandler(async (req, res: Response) => {
    ...
    const user = db.prepare('SELECT association_id FROM users WHERE id = ?')
        .get(oauthState.user_id) as { association_id: string } | undefined;
    ...
    const association = db.prepare(`
        SELECT google_calendar_client_id, google_calendar_client_secret
        FROM associations WHERE id = ?
    `).get(user.association_id) as { google_calendar_client_id: string; google_calendar_client_secret: string } | undefined;
 ...
 *     tags: [Calendar]
 */
router.post('/google/disconnect', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    db.prepare(`
        UPDATE user_calendar_settings
 ...
 *     tags: [Calendar]
 */
router.post('/google/sync', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    const settings = db.prepare(`
        SELECT * FROM user_calendar_settings WHERE user_id = ?
    `).get(req.user!.id) as CalendarSettings | undefined;
