...
// Holiday data changes rarely (yearly sync + occasional custom holidays).
// Responses are association-scoped, so the cache varies by association (default).
const CACHE_PATH = '/api/holidays';
const holidaysCache = cacheMiddleware({ ttlSeconds: 600 });
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `,
    ).run(id, req.user!.associationId, name, region || null, startDate, endDate, year, holidayType || null);
