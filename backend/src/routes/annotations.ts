...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const annotation = db
      .prepare('SELECT id FROM pdf_annotations WHERE id = ? AND user_id = ?')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare('DELETE FROM pdf_annotations WHERE id = ? AND user_id = ?')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare('DELETE FROM pdf_annotations WHERE music_piece_id = ? AND user_id = ?')
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // Verify annotation exists and belongs to user
    const annotation = db.prepare('SELECT id FROM pdf_annotations WHERE id = ? AND user_id = ?').get(id, req.user!.id);
