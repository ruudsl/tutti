...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // Get instruments for all sections in one batch query
    const sectionIds = sections.map((s: any) => s.id);
    const instrumentsBySection = new Map<string, any[]>();
    ...
    if (sectionIds.length > 0) {
      ...
      const allInstruments = db
        .prepare(
          `
            SELECT i.id, i.name, i.tuning, ssi.sort_order, ssi.section_id
        ...
        .all(...sectionIds) as any[];

      for (const row of allInstruments) {
        const list = instrumentsBySection.get(row.section_id) || [];
        list.push(row);
        instrumentsBySection.set(row.section_id, list);
      }
    }

    const sectionsWithInstruments = sections.map((section: any) => {
      const instruments = instrumentsBySection.get(section.id) || [];
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // Verify orchestra belongs to association
    const orchestra = db
      .prepare('SELECT id, name FROM orchestras WHERE id = ? AND association_id = ?')
    ...
    const existingSections = db
      .prepare('SELECT COUNT(*) as count FROM seating_sections WHERE orchestra_id = ?')
    ...
    const defaultLayout = [
      {
        rowNumber: 1,
        name: 'Rij 1 - Fluiten, Hobo, Klarinet 1',
        instruments: ['Flute', 'Piccolo', 'Oboe', 'Clarinet'],
      },
      {
        rowNumber: 2,
        name: 'Rij 2 - Klarinetten 2 & 3',
        instruments: ['Bass Clarinet', 'Eb Clarinet', 'Alto Clarinet'],
      },
      {
        rowNumber: 3,
        name: 'Rij 3 - Saxofoons, Hoorns, Trompetten',
        instruments: [
          'Alto Saxophone',
          'Tenor Saxophone',
          'Baritone Saxophone',
      ...
      {
        rowNumber: 4,
        name: 'Rij 4 - Baritons, Bassen, Trombones',
        instruments: [
          'Euphonium',
          'Tuba',
          'Trombone',
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existingRow = db
      .prepare('SELECT id FROM seating_sections WHERE orchestra_id = ? AND row_number = ?')
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (data.rowNumber !== undefined) {
      const existingRow = db
        .prepare('SELECT id FROM seating_sections WHERE orchestra_id = ? AND row_number = ? AND id != ?')
 ...
 */
router.delete(
  '/sections/orchestra/:orchestraId',
  authenticateToken,
  requireRole('admin', 'conductor', 'music_committee'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { orchestraId } = req.params;
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existingAssignment = db
      .prepare('SELECT id FROM seating_assignments WHERE orchestra_id = ? AND user_id = ?')
 ...
 */
router.put(
  '/assignments/bulk/:orchestraId',
  authenticateToken,
  requireRole('admin', 'conductor', 'music_committee'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { orchestraId } = req.params;
    const { assignments } = req.body as {
      assignments: { userId: string; sectionId: string; positionInSection: number }[];
    };
 ...
 */
router.post(
  '/rehearsal/:rehearsalId/generate',
  authenticateToken,
  requireRole('admin', 'conductor', 'music_committee'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { rehearsalId } = req.params;
    ...
    if (!orchestraId) {
      ...
      const orchestraWithSections = db
        .prepare(
          `
    ...
    const existingAssignments = orchestraId
      ? (db
          .prepare(
            `
        SELECT sa.user_id, sa.section_id, sa.position_in_section
 ...
 */
router.put(
  '/rehearsal/:rehearsalId/seat/:seatId',
  authenticateToken,
  requireRole('admin', 'conductor', 'music_committee'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
