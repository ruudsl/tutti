...
import {
  createMusicListSchema,
  updateMusicListSchema,
  reorderMusicListsSchema,
  addPieceToListSchema,
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const lists = db
      .prepare(
        ...
        WHERE uo.user_id = ? AND o.association_id = ? AND ml.deleted_at IS NULL ${activeFilter}
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id FROM music_lists WHERE LOWER(name) = LOWER(?) AND orchestra_id = ? AND deleted_at IS NULL')
    ...
    // Get next position
    const maxPos = db
      .prepare('SELECT COALESCE(MAX(position), -1) + 1 as next_pos FROM music_lists WHERE orchestra_id = ?')
      .get(data.orchestraId) as { next_pos: number };

    const listId = uuidv4();
    db.prepare(
      'INSERT INTO music_lists (id, name, orchestra_id, position, list_type, concert_date, concert_location) VALUES (?, ?, ?, ?, ?, ?, ?)',
    ).run(
      listId,
      data.name.trim(),
      data.orchestraId,
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare(
        'SELECT id FROM music_lists WHERE LOWER(name) = LOWER(?) AND orchestra_id = ? AND deleted_at IS NULL AND id != ?',
      )
    ...
    db.prepare(
      'UPDATE music_lists SET name = ?, list_type = ?, concert_date = ?, concert_location = ? WHERE id = ?',
    ).run(
      data.name.trim(),
      data.listType || 'regular',
      data.concertDate || null,
...
router.patch(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logger.info(`Music list ${req.params.id} toggled to ${newStatus === 1 ? 'active' : 'inactive'}`, {
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    db.prepare('UPDATE music_lists SET deleted_at = ? WHERE id = ?').run(new Date().toISOString(), req.params.id);

    logger.info(`Music list soft-deleted: ${req.params.id}`, { deletedBy: req.user!.id });
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    db.prepare('INSERT INTO music_list_pieces (music_list_id, music_piece_id) VALUES (?, ?)').run(
      req.params.id,
      data.pieceId,
    );
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = db
      .prepare('DELETE FROM music_list_pieces WHERE music_list_id = ? AND music_piece_id = ?')
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const pieces = db
      .prepare('SELECT id FROM music_pieces WHERE title = ? AND association_id = ? AND deleted_at IS NULL')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    let removed = 0;
    withTransaction(() => {
      const remove = db.prepare('DELETE FROM music_list_pieces WHERE music_list_id = ? AND music_piece_id = ?');
