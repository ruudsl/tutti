...
function findOrCreateOrchestras(orchestraNames: string[], associationId: string | null): string[] {
  ...
  withTransaction(() => {
    for (const name of orchestraNames) {
      ...
      const orchestra = db
        .prepare('SELECT id FROM orchestras WHERE LOWER(name) = LOWER(?) AND association_id = ?')
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id FROM job_title_instrument_mappings WHERE association_id = ? AND job_title = ?')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare('DELETE FROM job_title_instrument_mappings WHERE id = ? AND association_id = ?')
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existingOrchestras = db
      .prepare('SELECT LOWER(name) as name FROM orchestras WHERE association_id = ?')
    ...
    const enrichedUsers = entraUsers
      .map((user) => {
    ...
    const newDepartments = Array.from(uniqueDepartments).filter((d) => !existingOrchestraNames.has(d.toLowerCase()));
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    withTransaction(() => {
      for (const entraUser of selectedUsers) {
        ...
        const existing = db
          .prepare('SELECT id FROM users WHERE (microsoft_id = ? OR LOWER(email) = ?) AND association_id = ?')
          .get(entraUser.id, email, req.user!.associationId) as { id: string } | undefined;

        if (existing) {
          // Update existing user with Microsoft ID if not set
          db.prepare('UPDATE users SET microsoft_id = COALESCE(microsoft_id, ?) WHERE id = ?').run(
            entraUser.id,
            existing.id,
          );
          results.skipped++;
        ...
        if (entraUser.jobTitle) {
          ...
          if (instrumentId) {
            db.prepare('INSERT OR IGNORE INTO user_instruments (user_id, instrument_id) VALUES (?, ?)').run(
              userId,
              instrumentId,
            );
          }
        ...
        logger.info(`User imported from Entra ID: ${email}`, {
          userId,
    ...
    withTransaction(() => {
      for (const entraUser of entraUsers) {
        ...
        if (existing) {
          ...
          results.created++;
          logger.info(`User created from Entra sync: ${email}`, {
            userId,
            microsoftId: entraUser.id,
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (users.length === 0) {
      return res.json({
        message:
          'Geen gebruikers met Microsoft ID gevonden. Sync eerst gebruikers via "Sync bestaande" of importeer nieuwe gebruikers.',
        synced: 0,
        skipped: 0,
        failed: 0,
      });
