...
async function downloadAndSave(url: string, accessToken: string | null, originalName: string): Promise<{ filename: string; filePath: string }> {
    ...
    if (uploaded.length > 0) {
        ...
        if (listId) {
            ...
            if (musicList?.orchestra_id) {
                notifyOrchestra(musicList.orchestra_id, 'new_music', notifTitle, notifBody, notifData, user.id)
            ...
            ).all(user.associationId) as { id: string }[];
            for (const orch of orchestras) {
                notifyOrchestra(orch.id, 'new_music', notifTitle, notifBody, notifData, user.id)
                    .catch((err: Error) => logger.error('Failed to send new_music notification', { error: err.message }));
            }
        }
    }
 ...
 *       - bearerAuth: []
 */
router.post('/onedrive', authenticateToken, requireRole('admin', 'music_committee'), asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const downloadFiles = files.map(f => ({
        downloadUrl: f.downloadUrl || `https://graph.microsoft.com/v1.0/me/drive/items/${encodeURIComponent(f.id)}/content`,
        name: f.name,
 ...
 *       - bearerAuth: []
 */
router.post('/google-drive', authenticateToken, requireRole('admin', 'music_committee'), asyncHandler(async (req: AuthRequest, res: Response) => {
    const { files, accessToken, listId } = req.body as {
        files: CloudFile[];
        accessToken: string;
        listId?: string;
 ...
 *       - bearerAuth: []
 */
router.get('/config', authenticateToken, asyncHandler(async (_req: AuthRequest, res: Response) => {
    ...
    res.json({
        ...
        googleDrive: {
            enabled: !!(association?.google_drive_enabled && association?.google_drive_client_id && association?.google_drive_api_key),
