...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id FROM post_categories WHERE association_id = ? AND slug = ?')
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const category = db
      .prepare('SELECT id FROM post_categories WHERE id = ? AND association_id = ?')
    ...
    if (data.slug) {
      const existing = db
        .prepare('SELECT id FROM post_categories WHERE association_id = ? AND slug = ? AND id != ?')
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const visiblePosts = posts.filter((p: any) => isAdmin || canUserSeePost(p, req.user!));

    // Get categories and read status for all visible posts in two batch queries
    ...
    if (postIds.length > 0) {
      ...
      const allCategories = db
        .prepare(
          `
            SELECT pc.id, pc.name, pc.slug, pc.color, pcm.post_id
            FROM post_categories pc
            JOIN post_category_mapping pcm ON pc.id = pcm.category_id
            WHERE pcm.post_id IN (${placeholders})
        `,
        )
        .all(...postIds) as any[];
      ...
      const readRows = db
        ...
        .all(req.user!.id, ...postIds) as any[];
    ...
    const result = visiblePosts.map((p: any) => {
      const categories = categoriesByPost.get(p.id) || [];
      const isRead = readPostIds.has(p.id);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existingSlug = db
      .prepare('SELECT id FROM posts WHERE association_id = ? AND slug = ?')
    ...
    if (data.categoryIds && data.categoryIds.length > 0) {
      const insertCategory = db.prepare('INSERT INTO post_category_mapping (post_id, category_id) VALUES (?, ?)');
      data.categoryIds.forEach((catId) => {
        insertCategory.run(postId, catId);
      });
    }

    logAuditEvent(req.user!.id, 'post_created', 'post', postId, data.title);
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const post = db
      .prepare('SELECT * FROM posts WHERE id = ? AND association_id = ?')
    ...
    if (data.slug !== undefined) {
      const existingSlug = db
        .prepare('SELECT id FROM posts WHERE association_id = ? AND slug = ? AND id != ?')
        .get(associationId, data.slug, req.params.id);
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logAuditEvent(req.user!.id, 'post_deleted', 'post', req.params.id, post.title);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (data.parentId) {
      const parent = db
        .prepare('SELECT id FROM post_comments WHERE id = ? AND post_id = ? AND deleted_at IS NULL')
...
export async function processScheduledPosts(): Promise<number> {
  ...
  for (const post of scheduledPosts) {
    ...
    logAuditEvent(post.created_by, 'post_auto_published', 'post', post.id, `Automatisch gepubliceerd: ${post.title}`);
