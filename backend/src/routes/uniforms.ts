 ...
 *     tags: [Uniforms]
 */
router.get('/size-search', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *     tags: [Uniforms]
 */
router.get('/available-by-size', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *     tags: [Uniforms]
 */
router.get('/items', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *     tags: [Uniforms]
 */
router.get('/items/:id', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.post('/items', authenticateToken, requireRole('admin', 'music_committee'), asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `).run(
        id, req.user!.associationId, data.itemType, data.sizeStandard || null,
        data.sizeLength || null, data.sizeWidth || null, data.color || null,
        data.condition, data.status, data.currentUserId || null, data.notes || null,
    ...
    withTransaction(() => {
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            stmt.run(
                id, req.user!.associationId, data.itemType, data.sizeStandard || null,
                data.sizeLength || null, data.sizeWidth || null, data.color || null,
                data.condition, data.status, data.notes || null,
                data.purchaseDate || null, data.purchasePrice || null
            );
        }
    });
...
router.put('/items/:id', authenticateToken, requireRole('admin', 'music_committee'), asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    db.prepare(`
        ...
        WHERE id = ?
    `).run(
        data.itemType, data.sizeStandard, data.sizeLength, data.sizeWidth,
        data.color, data.condition, data.status,
        data.currentUserId !== undefined ? data.currentUserId : (existing as any).current_user_id,
        data.notes, data.purchaseDate, data.purchasePrice,
        req.params.id
    );
 ...
 *     tags: [Uniforms]
 */
router.delete('/items/:id', authenticateToken, requireRole('admin'), asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db.prepare('DELETE FROM uniform_items WHERE id = ? AND association_id = ?')
...
router.post('/items/:id/assign', authenticateToken, requireRole('admin', 'music_committee'), asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    withTransaction(() => {
        ...
        db.prepare('UPDATE uniform_items SET status = ?, current_user_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
            .run('issued', data.userId, req.params.id);
    });

    logger.info(`Uniform item assigned: ${req.params.id}`, { assignmentId: id, userId: data.userId, assignedBy: req.user!.id });
 ...
 *     tags: [Uniforms]
 */
router.get('/sets', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *     tags: [Uniforms]
 */
router.get('/sets/:id', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *     tags: [Uniforms]
 */
router.delete('/sets/:id', authenticateToken, requireRole('admin'), asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db.prepare('DELETE FROM uniform_sets WHERE id = ? AND association_id = ?')
        .run(req.params.id, req.user!.associationId);
 ...
 *     tags: [Uniforms]
 */
router.get('/user/:userId', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    const items = db.prepare(`
        SELECT ui.*
        FROM uniform_items ui
