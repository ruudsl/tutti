...
const DEFAULT_MOLLIE_FEES: Record<string, number> = {
  ideal: 0.35,
  creditcard: 0.3, // + percentage, but we'll use fixed for simplicity
  bancontact: 0.39,
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const activeProfileId = mode === 'test' ? settings.mollie_test_profile_id : settings.mollie_profile_id;
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const keyForMode = mode === 'test' ? row?.mollie_test_api_key_encrypted : row?.mollie_api_key_encrypted;

    if (!keyForMode) {
      throw new ApiError(
        400,
        mode === 'test' ? 'Geen test API-sleutel geconfigureerd.' : 'Geen live API-sleutel geconfigureerd.',
      );
    }
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const activeKey = row.mollie_mode === 'test' ? row.mollie_test_api_key_encrypted : row.mollie_api_key_encrypted;

    if (!activeKey) {
      const otherMode = row.mollie_mode === 'test' ? 'live' : 'test';
      const otherKey = otherMode === 'test' ? row.mollie_test_api_key_encrypted : row.mollie_api_key_encrypted;
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const mode = (settings?.mollie_mode as 'live' | 'test') || 'live';
    const encryptedKey = mode === 'test' ? settings?.mollie_test_api_key_encrypted : settings?.mollie_api_key_encrypted;
    ...
    if (isEncrypted(encryptedKey)) {
      ...
      db.prepare(`UPDATE payment_settings SET ${column} = ? WHERE association_id = ?`).run(
        properlyEncrypted,
        associationId,
      );
    }
