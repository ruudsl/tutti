...
router.get('/policies', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const countResult = db.prepare(`SELECT COUNT(*) as total FROM instrument_insurance_policies p ${whereClause}`).get(...params) as { total: number };
...
router.get('/policies/summary', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const uninsuredAssets = db.prepare(`
        SELECT COUNT(*) as count
...
router.get('/policies/expiring', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.get('/policies/:id', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.post('/policies', authenticateToken, requireRole('admin'), asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `).run(
        id, req.user!.associationId, data.policyNumber, data.providerName,
        data.providerContact || null, data.providerPhone || null, data.providerEmail || null,
        data.policyType, data.coverageType, data.coverageAmount, data.deductible,
        data.currency, data.premiumAmount || null, data.premiumFrequency || null,
        data.premiumDueDate || null, data.startDate, data.endDate || null,
        data.autoRenew ? 1 : 0, data.coverageDetails || null, data.exclusions || null,
        data.documentUrl || null, req.user!.id
    );
...
router.put('/policies/:id', authenticateToken, requireRole('admin'), asyncHandler(async (req: AuthRequest, res: Response) => {
    const data = updatePolicySchema.parse(req.body);

    const existing = db.prepare('SELECT id FROM instrument_insurance_policies WHERE id = ? AND association_id = ?')
    ...
    db.prepare(`
        ...
        WHERE id = ?
    `).run(
        data.policyNumber, data.providerName, data.providerContact, data.providerPhone,
        data.providerEmail, data.policyType, data.coverageType, data.coverageAmount,
        data.deductible, data.premiumAmount, data.premiumFrequency, data.premiumDueDate,
        data.startDate, data.endDate, data.autoRenew !== undefined ? (data.autoRenew ? 1 : 0) : null,
        data.coverageDetails, data.exclusions, data.documentUrl, req.params.id
    );

    res.json({ message: 'Verzekeringspolis bijgewerkt' });
}));

router.delete('/policies/:id', authenticateToken, requireRole('admin'), asyncHandler(async (req: AuthRequest, res: Response) => {
    const policy = db.prepare('SELECT id FROM instrument_insurance_policies WHERE id = ? AND association_id = ?')
        .get(req.params.id, req.user!.associationId);
    ...
    db.prepare('UPDATE instrument_assets SET insurance_policy_id = NULL WHERE insurance_policy_id = ?').run(req.params.id);
...
router.post('/policies/:policyId/coverage', authenticateToken, requireRole('admin'), asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, req.params.policyId, data.assetId, data.coveredAmount, data.coverageStart, data.coverageEnd || null, data.specialConditions || null);

    db.prepare('UPDATE instrument_assets SET insurance_policy_id = ? WHERE id = ?').run(req.params.policyId, data.assetId);

    res.status(201).json({ id, message: 'Instrument toegevoegd aan polis' });
}));

router.delete('/policies/:policyId/coverage/:coverageId', authenticateToken, requireRole('admin'), asyncHandler(async (req: AuthRequest, res: Response) => {
    const coverage = db.prepare(`
    ...
    db.prepare('UPDATE instrument_assets SET insurance_policy_id = NULL WHERE id = ? AND insurance_policy_id = ?')
        .run(coverage.asset_id, req.params.policyId);
...
router.get('/claims', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    const { page, limit, offset } = getPaginationParams(req.query);
    const status = req.query.status as string | undefined;
...
router.get('/claims/:id', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.post('/claims', authenticateToken, requireRole('admin', 'equipment_committee'), asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `).run(
        id, data.policyId, data.assetId, data.claimNumber || null, data.claimDate,
        data.incidentDate, data.incidentType, data.incidentDescription,
        data.incidentLocation || null, data.claimedAmount || null,
        data.policeReportNumber || null, data.witnessInfo || null,
        data.photos ? JSON.stringify(data.photos) : null, req.user!.id
    );

    logger.info(`Insurance claim created: ${id}`, { policyId: data.policyId, assetId: data.assetId, createdBy: req.user!.id });

    res.status(201).json({ id, message: 'Schadeclaim ingediend' });
}));

router.put('/claims/:id', authenticateToken, requireRole('admin'), asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    db.prepare(`
        ...
        WHERE id = ?
    `).run(
        data.status, data.approvedAmount, data.paidAmount, data.resolutionDate,
        data.resolutionNotes, data.documentUrls ? JSON.stringify(data.documentUrls) : null,
        req.params.id
    );
