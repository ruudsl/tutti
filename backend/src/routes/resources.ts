...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = db.prepare(
      'DELETE FROM resource_categories WHERE id = ? AND association_id = ?'
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `).run(
      ...
      data.resourceType, data.location || null, data.capacity || null,
      data.requiresApproval ? 1 : 0, data.minBookingHours || null, data.maxBookingHours || null,
      data.advanceBookingDays || null, data.costPerHour || null, data.costPerDay || null,
      data.depositAmount || null, data.notes || null
    );
...
router.patch(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const resource = db.prepare(
      'SELECT id FROM resources WHERE id = ? AND association_id = ? AND deleted_at IS NULL'
    ...
    if (data.resourceType !== undefined) { updates.push('resource_type = ?'); params.push(data.resourceType); }
    ...
    if (data.requiresApproval !== undefined) { updates.push('requires_approval = ?'); params.push(data.requiresApproval ? 1 : 0); }
    if (data.minBookingHours !== undefined) { updates.push('min_booking_hours = ?'); params.push(data.minBookingHours); }
    if (data.maxBookingHours !== undefined) { updates.push('max_booking_hours = ?'); params.push(data.maxBookingHours); }
    if (data.advanceBookingDays !== undefined) { updates.push('advance_booking_days = ?'); params.push(data.advanceBookingDays); }
    if (data.costPerHour !== undefined) { updates.push('cost_per_hour = ?'); params.push(data.costPerHour); }
    if (data.costPerDay !== undefined) { updates.push('cost_per_day = ?'); params.push(data.costPerDay); }
    if (data.depositAmount !== undefined) { updates.push('deposit_amount = ?'); params.push(data.depositAmount); }
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      availId, id, data.availabilityType, data.dayOfWeek ?? null,
      data.startTime || null, data.endTime || null, data.startDate || null,
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id, data.resourceId, userId, data.title, data.description || null,
      data.startDatetime, data.endDatetime, status, data.relatedRehearsalId || null,
      data.relatedConcertId || null, data.relatedProjectId || null, data.notes || null
    );
