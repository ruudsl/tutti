...
router.get('/', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (!typeFilter || typeFilter === 'orchestra') {
    const orchestraResults = db.prepare(`
      SELECT o.id, o.name,
  ...
  if (!typeFilter || typeFilter === 'rehearsal') {
    const rehearsalResults = db.prepare(`
  ...
  if (!typeFilter || typeFilter === 'settings') {
    const settingsItems = [
      ...
      { id: 'settings', title: 'Instellingen', path: '/settings', keywords: ['instellingen', 'settings', 'configuratie'] },
      { id: 'theme', title: 'Thema', path: '/theme', keywords: ['thema', 'kleuren', 'theme', 'donker', 'licht', 'dark', 'light'] },
      { id: 'upload', title: 'Muziek uploaden', path: '/upload', keywords: ['upload', 'uploaden', 'nieuw', 'toevoegen'] },
 ...
 */
router.get('/suggestions', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
  const query = (req.query.q as string || '').trim().toLowerCase();
  const associationId = req.user!.associationId;
  ...
  const musicSuggestions = db.prepare(`
  ...
  const memberSuggestions = db.prepare(`
 ...
 */
router.post('/recent', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  db.prepare('DELETE FROM user_recent_searches WHERE user_id = ? AND LOWER(query) = LOWER(?)').run(userId, trimmedQuery);
 ...
 */
router.delete('/recent/:id', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const userId = req.user!.id;
 ...
 */
router.delete('/recent', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user!.id;
