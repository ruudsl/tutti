 ...
 *       - bearerAuth: []
 */
router.put('/preferences', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    const {
        emailEnabled,
        pushEnabled,
        whatsappEnabled,
 ...
 *       - bearerAuth: []
 */
router.put('/preferences/type/:type', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    const { type } = req.params;
    const { channels } = req.body;

    const validTypes: NotificationType[] = [
        'new_music', 'rehearsal_change', 'seating_update',
        'chat_message', 'practice_reminder', 'concert_reminder',
 ...
 *       - bearerAuth: []
 */
router.post('/telegram/link', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *       - bearerAuth: []
 */
router.get('/telegram/status', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *       - bearerAuth: []
 */
router.delete('/telegram/unlink', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    db.prepare(`
 ...
 *       - bearerAuth: []
 */
router.post('/whatsapp/link', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 *       - bearerAuth: []
 */
router.post('/whatsapp/verify', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    const { code } = req.body;
 ...
 *       - bearerAuth: []
 */
router.get('/whatsapp/status', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    const channel = db.prepare(`
        SELECT channel_id, verified, created_at FROM user_notification_channels
        WHERE user_id = ? AND channel_type = 'whatsapp'
    `).get(req.user!.id) as { channel_id: string; verified: number; created_at: string } | undefined;
 ...
 *       - bearerAuth: []
 */
router.delete('/whatsapp/unlink', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    db.prepare(`
        DELETE FROM user_notification_channels
        WHERE user_id = ? AND channel_type = 'whatsapp'
    `).run(req.user!.id);
