...
import { z } from 'zod';
import { generateSecureTicketCode, generateQRCode, sendTicketConfirmationEmail } from '../services/ticketing';
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `,
      ...
      { id: string; name: string; date: string; location: string | null } | undefined;
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (existing.tickets_sent === 1) {
      throw new ApiError(
        400,
        'Cannot delete entry after tickets have been sent. Consider cancelling the tickets instead.',
      );
    }
