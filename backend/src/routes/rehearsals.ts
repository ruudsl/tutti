...
function getUserOrchestraIds(userId: string): string[] {
  const rows = db.prepare('SELECT orchestra_id FROM user_orchestras WHERE user_id = ?').all(userId) as {
    orchestra_id: string;
  }[];
...
router.post(
  '/default-days',
  authenticateToken,
  requireRole(...REHEARSAL_MANAGERS),
  asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id FROM rehearsal_default_days WHERE id = ? AND association_id = ?')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = db
      .prepare('DELETE FROM rehearsal_default_days WHERE id = ? AND association_id = ?')
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (orchestraId) {
      notifyOrchestra(orchestraId, 'rehearsal_change', notifTitle, notifBody, notifData, req.user!.id).catch(
        (err: Error) => logger.error('Failed to send rehearsal_change notification', { error: err.message }),
      );
    } else {
      ...
      for (const orch of orchestras) {
        notifyOrchestra(orch.id, 'rehearsal_change', notifTitle, notifBody, notifData, req.user!.id).catch(
          (err: Error) => logger.error('Failed to send rehearsal_change notification', { error: err.message }),
        );
      }
    }
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id, orchestra_id, date as existing_date FROM rehearsals WHERE id = ? AND association_id = ?')
      .get(id, req.user!.associationId) as any;
    if (!existing) {
      throw new ApiError(404, 'Repetitie niet gevonden.');
    }
