 ...
 */
router.get('/detail', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    $('table tr').each((_, row) => {
        ...
        if (cells.length >= 2) {
            ...
            } else if (label.includes('duration') || label.includes('dauer') || label.includes('speelduur') || label.includes('playing time')) {
                result.duration = value;
                result.durationSeconds = parseDurationToSeconds(value);
            } else if (label.includes('difficulty') || label.includes('schwierigkeit') || label.includes('moeilijk') || label.includes('grade')) {
    ...
    if (!result.duration) {
        ...
        const durationMatch = bodyText.match(/(?:Duration|Dauer|Speelduur|Playing\s*time)[:\s]*(\d{1,2}:\d{2}(?::\d{2})?)/i);
        if (durationMatch) {
