...
// inventory and payment initiation calls out to the payment provider. Uses
// req.ip, which respects the app's 'trust proxy' setting behind Render/Nginx.
// Together with the CAPTCHA layer (services/captcha) this replaces the former
// custom middleware/checkoutRateLimit.ts.
const checkoutStartLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 5,
  message: { error: 'Te veel bestelpogingen. Probeer het over een minuut opnieuw.', code: 'RATE_LIMITED' },
...
const paymentLimiter = rateLimit({
  ...
  max: 10,
  message: { error: 'Te veel betaalpogingen. Probeer het later opnieuw.', code: 'RATE_LIMITED' },
  standardHeaders: true,
  legacyHeaders: false,
});
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      ...
      ticketTypes: ticketTypes.map((tt) => ({
        ...
        onSale: (!tt.sale_start || tt.sale_start <= now) && (!tt.sale_end || tt.sale_end >= now),
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (provider === 'mollie') {
      ...
      if (!Buffer.isBuffer(req.body)) {
        ...
        logger.error(
          'Stripe webhook received a parsed body instead of raw bytes - check express.raw() mounting for /api/tickets/webhooks/payment',
        );
        ...
        for (let i = 0; i < item.quantity; i++) {
          ...
          db.prepare(
            `
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, 'valid')
                    `,
          ).run(ticketId, item.ticket_type_id, orderId, order.user_id, order.buyer_name, order.buyer_email, qrCode);
        }
      }
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const orderItemsMap: Record<string, { ticketTypeId: string; name: string; quantity: number; unitPrice: number }[]> =
      {};
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const { name, price, quantity, description, saleStart, saleEnd, maxPerOrder, serviceFee, showServiceFeeSeparate } =
      validation.data;
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const seatData: SeatDataItem[] = soldTickets.map((ticket) => {
      ...
      const salesSpeedPercentile =
        sectionTickets.length > 1 ? Math.round((1 - positionInSection / (sectionTickets.length - 1)) * 100) : 50;
 ...
 */
router.post(
  '/tickets/orders/:id/mock-payment',
  authenticateToken,
  requireRole('admin'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
