 ...
 */
router.get('/templates', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
 ...
 */
router.post('/templates', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
  const {
    name,
    description,
    defaultRehearsalDay,
 ...
 */
router.put('/templates/:id', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const {
    name,
    description,
  ...
  const existing = db.prepare('SELECT id FROM season_templates WHERE id = ? AND association_id = ?')
 ...
 */
router.delete('/templates/:id', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;

  const result = db.prepare('DELETE FROM season_templates WHERE id = ? AND association_id = ?')
 ...
 */
router.post('/', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (templateId) {
    const template = db.prepare('SELECT id FROM season_templates WHERE id = ? AND association_id = ?')
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, req.user!.associationId, name, startDate, endDate, templateId || null, budgetTotal || null, notes || null, req.user!.id);
 ...
 */
router.put('/:id', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const { name, startDate, endDate, templateId, status, budgetTotal, budgetAllocated, notes } = req.body;

  const existing = db.prepare('SELECT id FROM seasons WHERE id = ? AND association_id = ?')
    .get(id, req.user!.associationId);
 ...
 */
router.delete('/:id', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;

  const result = db.prepare('DELETE FROM seasons WHERE id = ? AND association_id = ?')
    .run(id, req.user!.associationId);
 ...
 */
router.post('/:id/events', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const { eventType, eventId, plannedDate, budgetAmount, notes } = req.body;

  const season = db.prepare('SELECT id FROM seasons WHERE id = ? AND association_id = ?')
    .get(id, req.user!.associationId);
 ...
 */
router.delete('/:id/events/:eventId', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const event = db.prepare('SELECT budget_amount FROM season_events WHERE id = ? AND season_id = ?')
 ...
 */
router.post('/:id/generate', authenticateToken, requireRole(...SEASON_MANAGERS), asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const {
    rehearsalDay,
    rehearsalTime,
      ...
      while (current <= end) {
        ...
        if (!existingSet.has(dateStr) && !excludeSet.has(dateStr)) {
          ...
          db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 'regular', ?)
          `).run(rehearsalId, req.user!.associationId, orchestraId || null, dateStr, time, endTimeCalc, location || null, req.user!.id);
  ...
  if (generateConcerts && concerts && Array.isArray(concerts)) {
    for (const concert of concerts) {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(concertId, req.user!.associationId, concert.name, concert.date, concert.location || null, concert.type || null, req.user!.id);
