  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (platform === 'spotify') {
      ...
      if (!appleMusicClient.isConfigured()) {
        throw new ApiError(
          503,
          'Apple Music API is not configured. Set APPLE_MUSIC_TEAM_ID, APPLE_MUSIC_KEY_ID, and APPLE_MUSIC_PRIVATE_KEY environment variables.',
        );
      }
 ...
 */
router.get(
  '/music-titles/:id/links',
  authenticateToken,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;

    const title = db
      .prepare('SELECT streaming_links FROM music_titles WHERE id = ? AND association_id = ?')
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const updatedLinks: StreamingLinks = {
      ...
      apple_music_preview_url:
        apple_music_preview_url !== undefined ? apple_music_preview_url : existingLinks.apple_music_preview_url,
    };
 ...
 */
router.delete(
  '/music-titles/:id/links/:platform',
  authenticateToken,
  requireRole('admin', 'music_committee'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
