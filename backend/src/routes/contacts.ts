...
router.post(
  ...
  cacheInvalidator(CATEGORIES_CACHE_PATH),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id FROM contact_categories WHERE association_id = ? AND LOWER(name) = LOWER(?)')
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const category = db
      .prepare('SELECT id FROM contact_categories WHERE id = ? AND association_id = ?')
    ...
    if (data.name) {
      const existing = db
        .prepare('SELECT id FROM contact_categories WHERE association_id = ? AND LOWER(name) = LOWER(?) AND id != ?')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = db
      .prepare('DELETE FROM contact_categories WHERE id = ? AND association_id = ?')
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // Get categories for all contacts in one batch query
    ...
    if (contactIds.length > 0) {
      ...
      const allCategories = db
        .prepare(
          `
            SELECT cc.id, cc.name, cc.color, cc.icon, ccl.contact_id
            FROM contact_categories cc
            JOIN contact_category_links ccl ON cc.id = ccl.category_id
            WHERE ccl.contact_id IN (${placeholders})
        `,
        )
        .all(...contactIds) as any[];

      for (const row of allCategories) {
        const list = categoriesByContact.get(row.contact_id) || [];
        list.push({ id: row.id, name: row.name, color: row.color, icon: row.icon });
        categoriesByContact.set(row.contact_id, list);
      }
    }

    const result = contacts.map((c) => {
      const categories = categoriesByContact.get(c.id) || [];
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    withTransaction(() => {
      ...
      if (data.categoryIds && data.categoryIds.length > 0) {
        const insertLink = db.prepare('INSERT INTO contact_category_links (contact_id, category_id) VALUES (?, ?)');
...
router.patch(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const contact = db
      .prepare('SELECT id, name FROM contacts WHERE id = ? AND association_id = ? AND deleted_at IS NULL')
      .get(req.params.id, associationId) as any;
    ...
    withTransaction(() => {
      ...
      if (data.addressLine !== undefined) {
        ...
        values.push(data.addressLine || null);
      }
      ...
      if (data.iban !== undefined) {
        updates.push('iban = ?');
        values.push(data.iban || null);
      }
      if (data.ibanHolderName !== undefined) {
        updates.push('iban_holder_name = ?');
        values.push(data.ibanHolderName || null);
      }
      if (data.bic !== undefined) {
        updates.push('bic = ?');
        values.push(data.bic || null);
      }
      if (data.vatNumber !== undefined) {
        updates.push('vat_number = ?');
        values.push(data.vatNumber || null);
      }
      if (data.chamberOfCommerce !== undefined) {
        updates.push('chamber_of_commerce = ?');
        values.push(data.chamberOfCommerce || null);
      }
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    db.prepare('UPDATE contacts SET deleted_at = CURRENT_TIMESTAMP, is_active = 0 WHERE id = ?').run(req.params.id);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const contact = db
      .prepare('SELECT id FROM contacts WHERE id = ? AND association_id = ? AND deleted_at IS NULL')
      .get(req.params.id, associationId);
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const person = db
      .prepare('SELECT id FROM contact_persons WHERE id = ? AND contact_id = ?')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const result = db
      .prepare('DELETE FROM contact_persons WHERE id = ? AND contact_id = ?')
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    withTransaction(() => {
      ...
      db.prepare('UPDATE contacts SET promoted_to_user_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(
        userId,
        contact.id,
      );
    });

    logger.info(`Contact promoted to user: ${contact.name}`, {
