...
// createEventSchema is defined centrally in ../validation/schemas.ts
...
// updateEventAttendanceSchema is defined centrally in ../validation/schemas.ts
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const countResult = db.prepare(`SELECT COUNT(*) as total FROM event_locations ${whereClause}`).get(...params) as {
      total: number;
    };
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id FROM event_locations WHERE id = ? AND association_id = ?')
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare('DELETE FROM event_locations WHERE id = ? AND association_id = ?')
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const countResult = db.prepare(`SELECT COUNT(*) as total FROM events e ${whereClause}`).get(...params) as {
      total: number;
    };
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT id FROM events WHERE id = ? AND association_id = ?')
      .get(req.params.id, req.user!.associationId);
    ...
    withTransaction(() => {
      ...
      const fieldMap: Record<string, string> = {
        ...
        isPublic: 'is_public',
        requiresTickets: 'requires_tickets',
        weatherSensitive: 'weather_sensitive',
        backupLocationId: 'backup_location_id',
        backupLocationName: 'backup_location_name',
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare('DELETE FROM events WHERE id = ? AND association_id = ?')
      .run(req.params.id, req.user!.associationId);
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const fieldMap: Record<string, string> = {
      ...
      departureLatitude: 'departure_latitude',
      departureLongitude: 'departure_longitude',
...
router.post(
  '/:eventId/transport/:transportId/passengers',
  authenticateToken,
  asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.delete(
  '/:eventId/transport/:transportId/passengers/:passengerId',
  authenticateToken,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare(
...
router.delete(
  '/:eventId/meeting-points/:pointId',
  authenticateToken,
  requireRole('board', 'admin'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
...
router.post(
  '/:eventId/packing-lists/:listId/items',
  authenticateToken,
  requireRole('board', 'admin'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const list = db
...
router.put(
  '/:eventId/packing-lists/:listId/items/:itemId',
  authenticateToken,
  asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.post(
  '/:eventId/attendance',
  authenticateToken,
  validate(updateEventAttendanceSchema),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const event = db
      .prepare('SELECT id FROM events WHERE id = ? AND association_id = ?')
      .get(req.params.eventId, req.user!.associationId);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    withTransaction(() => {
      if (isDefault) {
        db.prepare('UPDATE packing_list_templates SET is_default = 0 WHERE association_id = ?').run(
          req.user!.associationId,
        );
      }
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const result = db
      .prepare('DELETE FROM packing_list_templates WHERE id = ? AND association_id = ?')
      .run(req.params.id, req.user!.associationId);
