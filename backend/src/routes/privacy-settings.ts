...
router.get('/my-settings', authenticateToken, cacheMiddleware({ ttlSeconds: 60 }), asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.put('/my-settings', authenticateToken, cacheInvalidator(CACHE_PATH), asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    for (const setting of data.settings) {
        if (requiredFields.has(setting.fieldName)) {
            const associationDefault = db.prepare(`
            ...
            if (!allowedLevels.includes(setting.visibility)) {
                throw new ApiError(400, `Veld "${setting.fieldName}" moet minimaal ${associationDefault?.default_visibility || 'admin_only'} zichtbaarheid hebben.`);
            }
        }
    }
...
router.get('/defaults', authenticateToken, requireRole('admin'), cacheMiddleware({ ttlSeconds: 300 }), asyncHandler(async (req: AuthRequest, res: Response) => {
    const associationId = req.user!.associationId;
    if (!associationId) {
...
router.put('/defaults', authenticateToken, requireRole('admin'), cacheInvalidator(CACHE_PATH), asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.delete('/defaults/:fieldName', authenticateToken, requireRole('admin'), cacheInvalidator(CACHE_PATH), asyncHandler(async (req: AuthRequest, res: Response) => {
    const associationId = req.user!.associationId;
    if (!associationId) {
        throw new ApiError(400, 'Gebruiker heeft geen vereniging.');
    }
...
router.get('/consent', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    const userId = req.user!.id;
...
router.post('/consent', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.get('/consent/check/:version', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
    const userId = req.user!.id;
