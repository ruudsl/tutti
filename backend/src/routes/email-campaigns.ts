...
router.post(
  ...
  authenticateToken,
  requireRole('admin', 'music_committee'),
  cacheInvalidator(TEMPLATES_CACHE_PATH),
  asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const template = db
      .prepare('SELECT * FROM email_templates WHERE id = ? AND association_id = ?')
      .get(req.params.id, associationId) as any;
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logAuditEvent(req.user!.id, 'email_campaign_created', 'email_campaign', campaignId, data.name);
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const campaign = db
      .prepare('SELECT * FROM email_campaigns WHERE id = ? AND association_id = ?')
      .get(req.params.id, associationId) as any;
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logAuditEvent(req.user!.id, 'email_campaign_deleted', 'email_campaign', req.params.id, campaign.name);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logAuditEvent(req.user!.id, 'email_campaign_scheduled', 'email_campaign', req.params.id, campaign.name);
...
function getCampaignAttachments(campaignId: string): { filename: string; path: string; contentType: string }[] {
  const attachments = db
    .prepare('SELECT filename, original_filename, mime_type FROM email_campaign_attachments WHERE campaign_id = ?')
...
async function sendCampaign(campaignId: string, associationId: string): Promise<void> {
  ...
  logAuditEvent(campaign.created_by, 'email_campaign_sent', 'email_campaign', campaignId, campaign.name);
}
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const attachment = db
      .prepare('SELECT * FROM email_campaign_attachments WHERE id = ? AND campaign_id = ?')
