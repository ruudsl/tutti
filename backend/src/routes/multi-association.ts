...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (data.slug) {
      const slugExists = db
        .prepare('SELECT id FROM associations WHERE slug = ? AND id != ?')
        .get(data.slug, req.params.id);
      if (slugExists) {
        throw new ApiError(409, 'Slug is al in gebruik.');
      }
    }
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // Register a session for the newly issued token
    registerSession(updatedUser.id, token, req.ip, req.get('user-agent'));
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logActivity(req.user!.associationId, req.user!.id, 'invitation_sent', 'invitation', id, {
      email: data.email,
      role: data.role,
    });

    logger.info(`Invitation sent to ${data.email}`, {
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logActivity(invitation.association_id, req.user!.id, 'invitation_accepted', 'user', req.user!.id, {
      role: invitation.role,
    });
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json(
      (partnerships as any[]).map((p) => ({
        id: p.id,
        partnerAssociationId: p.association_a_id === req.user!.associationId ? p.association_b_id : p.association_a_id,
        partnerAssociationName:
          p.association_a_id === req.user!.associationId ? p.association_b_name : p.association_a_name,
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const targetAssoc = db
      .prepare('SELECT id, name FROM associations WHERE id = ?')
      .get(data.targetAssociationId) as any;
    ...
    logActivity(req.user!.associationId, req.user!.id, 'partnership_requested', 'partnership', id, {
      targetAssociationName: targetAssoc.name,
    });

    logger.info(`Partnership requested with ${targetAssoc.name}`, {
      from: req.user!.associationId,
      to: data.targetAssociationId,
    });
...
router.delete(
  '/events/:eventId/share/:associationId',
  authenticateToken,
  requireRole('board', 'admin'),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const event = db
...
function logActivity(
  ...
  db.prepare(
    `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `,
  ).run(uuidv4(), associationId, userId ?? null, action, entityType ?? null, entityId ?? null, JSON.stringify(details));
}
