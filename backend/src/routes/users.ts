...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const user = db
      .prepare('SELECT profile_photo_path FROM users WHERE id = ? AND association_id = ?')
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // Get instruments and orchestras for all users in two batch queries
    const userIds = users.map((u: any) => u.id);
    const instrumentsByUser = new Map<string, any[]>();
    const orchestrasByUser = new Map<string, any[]>();
    ...
    if (userIds.length > 0) {
      ...
      const allInstruments = db
        .prepare(
          `
            SELECT i.id, i.name, i.tuning, ui.user_id
            FROM instruments i
            JOIN user_instruments ui ON i.id = ui.instrument_id
            WHERE ui.user_id IN (${placeholders})
        `,
      ...
      for (const row of allInstruments) {
        const list = instrumentsByUser.get(row.user_id) || [];
        list.push({ id: row.id, name: row.name, tuning: row.tuning });
        instrumentsByUser.set(row.user_id, list);
      }

      const allOrchestras = db
        .prepare(
          `
            SELECT o.id, o.name, uo.user_id
            FROM orchestras o
            JOIN user_orchestras uo ON o.id = uo.orchestra_id
            WHERE uo.user_id IN (${placeholders})
        `,
      ...
      for (const row of allOrchestras) {
        const list = orchestrasByUser.get(row.user_id) || [];
        list.push({ id: row.id, name: row.name });
        orchestrasByUser.set(row.user_id, list);
      }
    }

    const result = users.map((user: any) => {
      const instruments = instrumentsByUser.get(user.id) || [];
      const orchestras = orchestrasByUser.get(user.id) || [];
...
// dus /users/export-data kwam uit bij '/:id', zocht een gebruiker met het id
// "export-data" en antwoordde met "Gebruiker niet gevonden". De export was
// daarmee onbereikbaar - terwijl dit de gegevensexport uit artikel 20 van de
// AVG is, die de roadmap als afgerond opvoert.
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    //
    // Deze query vroeg om action, details en ip_address. Die kolommen bestaan
    // niet in activity_log: die heeft action_type, entity_type, entity_id en
    // metadata, en houdt geen ip-adres bij. De export liep er dus op stuk met
    // "no such column: action". Dat viel niemand op omdat de route zelf
    // onbereikbaar was - hij stond onder '/:id' - dus deze fout kwam pas boven
    // water toen dat was rechtgezet.
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const user = db
      .prepare('SELECT id, profile_photo_path FROM users WHERE id = ? AND association_id = ?')
