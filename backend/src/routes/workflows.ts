...
const actionSchema = z.object({
  actionType: z.enum(['send_email', 'send_notification', 'create_task', 'update_field', 'add_to_group', 'remove_from_group', 'webhook', 'delay']),
...
router.patch('/:id', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const workflow = db.prepare(`SELECT id FROM workflows WHERE id = ? AND association_id = ? AND deleted_at IS NULL`).get(req.params.id, associationId);
  if (!workflow) {
    throw new ApiError(404, 'Workflow not found');
  }
  ...
  if (data.runOncePerEntity !== undefined) { updates.push('run_once_per_entity = ?'); values.push(data.runOncePerEntity ? 1 : 0); }
...
router.post('/:id/triggers', asyncHandler(async (req: AuthRequest, res: Response) => {
  const associationId = req.user!.associationId;
...
router.delete('/:id/triggers/:triggerId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const result = db.prepare(`DELETE FROM workflow_triggers WHERE id = ? AND workflow_id = ?`).run(req.params.triggerId, req.params.id);
...
router.post('/:id/actions', asyncHandler(async (req: AuthRequest, res: Response) => {
  const associationId = req.user!.associationId;
  ...
  const maxOrder = db.prepare(`SELECT COALESCE(MAX(action_order), -1) as max_order FROM workflow_actions WHERE workflow_id = ?`).get(req.params.id) as any;
...
router.delete('/:id/actions/:actionId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const result = db.prepare(`DELETE FROM workflow_actions WHERE id = ? AND workflow_id = ?`).run(req.params.actionId, req.params.id);
...
router.get('/:id/executions', asyncHandler(async (req: AuthRequest, res: Response) => {
  const associationId = req.user!.associationId;
  ...
  const total = db.prepare(`SELECT COUNT(*) as count FROM workflow_executions WHERE workflow_id = ?`).get(req.params.id) as any;
