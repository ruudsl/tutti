...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const countResult = db
      .prepare(`SELECT COUNT(*) as total FROM instrument_assets a ${whereClause}`)
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const insurance = row.insurance_policy_id
      ? db
          .prepare(
            `
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const existing = db
      .prepare('SELECT * FROM instrument_assets WHERE id = ? AND association_id = ? AND deleted_at IS NULL')
      .get(req.params.id, req.user!.associationId) as any;
    ...
    `,
      ...
      data.maintenanceIntervalMonths,
      nextMaintenanceDue,
      data.photoUrls ? JSON.stringify(data.photoUrls) : null,
      data.tags ? JSON.stringify(data.tags) : null,
      data.notes,
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `,
      ...
      data.valuedAmount,
      data.currency,
      data.appraiserName || null,
      data.appraiserCompany || null,
      data.appraiserCredentials || null,
      data.valuationMethod || null,
      data.marketComparison || null,
      data.conditionAtValuation || null,
      data.certificateUrl || null,
      data.notes || null,
      req.user!.id,
    );

    db.prepare('UPDATE instrument_assets SET current_value = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(
      data.valuedAmount,
      req.params.id,
    );
    ...
    logAssetHistory(
      req.params.id,
      'valuation_added',
      `Waardering toegevoegd: €${data.valuedAmount}`,
      req.user!.id,
      null,
      String(data.valuedAmount),
      'current_value',
      id,
      'valuation',
    );
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const asset = db
      .prepare('SELECT id FROM instrument_assets WHERE id = ? AND association_id = ?')
      .get(req.params.id, req.user!.associationId);
    ...
    const valuations = db
      .prepare('SELECT * FROM instrument_valuations WHERE asset_id = ? ORDER BY valuation_date DESC')
      .all(req.params.id);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    `,
      ...
      data.diagnosis || null,
      data.repairShopName || null,
      data.repairShopContact || null,
      data.repairShopAddress || null,
      data.technicianName || null,
      data.estimatedCost || null,
      data.estimatedCompletion || null,
      data.warrantyClaimRef ? 1 : 0,
    ...
    if (data.priority === 'urgent' || data.priority === 'high') {
      db.prepare('UPDATE instrument_assets SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(
        'in_repair',
        req.params.id,
      );
    }
    ...
    logAssetHistory(
      req.params.id,
      'repair_created',
      `Reparatie aangemeld: ${data.issueDescription.substring(0, 50)}`,
      req.user!.id,
      null,
      null,
      null,
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const repairs = db
      .prepare('SELECT * FROM instrument_repairs WHERE asset_id = ? ORDER BY reported_date DESC')
      .all(req.params.id);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    eisEigenLid(data.borrowerUserId, req.user!.associationId);
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    withTransaction(() => {
      db.prepare(
        ...
        data.returnInspectionNotes || null,
        data.damageReported || null,
        data.damageCost || null,
        req.user!.id,
        req.params.loanId,
      );
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logAssetHistory(
      req.params.id,
      'document_added',
      `Document toegevoegd: ${data.title}`,
      req.user!.id,
      null,
      null,
      null,
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const documents = db
      .prepare('SELECT * FROM instrument_documents WHERE asset_id = ? ORDER BY created_at DESC')
      .all(req.params.id);
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    logAssetHistory(
      ...
      `Onderhoud uitgevoerd${cost ? `: €${cost}` : ''}`,
      req.user!.id,
