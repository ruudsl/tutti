...
describe('Users Routes', () => {
  ...
  describe('GET /api/users', () => {
    ...
    it('should deny access to non-admin users', async () => {
      const response = await request(app).get('/api/users').set('Authorization', `Bearer ${memberToken}`);

      expect(response.status).toBe(403);
    });

    it('should deny access to music committee users', async () => {
      const response = await request(app).get('/api/users').set('Authorization', `Bearer ${musicCommitteeToken}`);
    ...
    it('should only return users from the same association', async () => {
      ...
      expect(response.body.data.every((u: any) => u.associationId === association.id)).toBe(true);
    });
  });
  ...
  describe('GET /api/users/:id', () => {
    ...
    it('should return 404 for user from different association', async () => {
      testDb.prepare('INSERT INTO associations (id, name) VALUES (?, ?)').run('other-assoc-2', 'Other Association 2');
  ...
  describe('POST /api/users', () => {
    it('should create a new user as admin', async () => {
      ...
      const response = await request(app).post('/api/users').set('Authorization', `Bearer ${adminToken}`).send(newUser);
    ...
    it('should create user with instruments and orchestras', async () => {
      ...
      const userInstruments = testDb.prepare('SELECT * FROM user_instruments WHERE user_id = ?').all(response.body.id);
      expect(userInstruments.length).toBe(1);

      const userOrchestras = testDb.prepare('SELECT * FROM user_orchestras WHERE user_id = ?').all(response.body.id);
    ...
    it('should deny access to non-admin users', async () => {
      const response = await request(app).post('/api/users').set('Authorization', `Bearer ${memberToken}`).send({
    ...
    it('should hash the password', async () => {
      ...
      const user = testDb.prepare('SELECT password_hash FROM users WHERE id = ?').get(response.body.id) as any;
  ...
  describe('PUT /api/users/:id', () => {
    ...
    it.skip('should update user as admin', async () => {
      ...
      // Verify the update
      const user = testDb.prepare('SELECT first_name, last_name FROM users WHERE id = ?').get(memberUser.id) as any;
    ...
    it.skip('should update user email', async () => {
      ...
      const user = testDb.prepare('SELECT email FROM users WHERE id = ?').get(memberUser.id) as any;
    ...
    it.skip('should update user role', async () => {
      ...
      const user = testDb.prepare('SELECT role FROM users WHERE id = ?').get(memberUser.id) as any;
    ...
    it('should return 404 for user from different association', async () => {
      testDb.prepare('INSERT INTO associations (id, name) VALUES (?, ?)').run('other-assoc-3', 'Other Association 3');
  ...
  describe('DELETE /api/users/:id', () => {
    it('should soft-delete user as admin', async () => {
      ...
      // Soft delete: row still exists, but is marked deleted and the
      // email address is mutated (original kept in email_before_delete)
      const user = testDb
        .prepare('SELECT id, deleted_at, status, email, email_before_delete FROM users WHERE id = ?')
        .get(userToDelete.id) as any;

      expect(user).toBeDefined();
      expect(user.deleted_at).not.toBeNull();
      expect(user.status).toBe('inactive');
      expect(user.email).toMatch(/^deleted-\d+-todelete@test\.com$/);
      expect(user.email_before_delete).toBe('todelete@test.com');

      // Soft-deleted user no longer shows up in the user list
      const listResponse = await request(app).get('/api/users').set('Authorization', `Bearer ${adminToken}`);
      expect(listResponse.status).toBe(200);
      expect(listResponse.body.data.some((u: any) => u.id === userToDelete.id)).toBe(false);
    ...
    it('should free up the email address for new users after soft-delete', async () => {
      const userToDelete = createTestUser(association.id, {
        email: 'reusable@test.com',
      });

      await request(app).delete(`/api/users/${userToDelete.id}`).set('Authorization', `Bearer ${adminToken}`);
    ...
    it('should return 404 for user from different association', async () => {
      testDb.prepare('INSERT INTO associations (id, name) VALUES (?, ?)').run('other-assoc-4', 'Other Association 4');
    ...
    it('should keep user instruments and orchestras until purge (soft delete)', async () => {
      const userToDelete = createTestUser(association.id, {
      ...
      // Soft delete keeps relations intact so a restore brings
      // everything back; they are removed at hard-delete (purge)
      const userInstruments = testDb.prepare('SELECT * FROM user_instruments WHERE user_id = ?').all(userToDelete.id);
      expect(userInstruments.length).toBe(1);

      const userOrchestras = testDb.prepare('SELECT * FROM user_orchestras WHERE user_id = ?').all(userToDelete.id);
      expect(userOrchestras.length).toBe(1);
    });
  });

  describe('POST /api/users/:id/restore', () => {
    it('should soft-delete and restore a user with original email', async () => {
      const user = createTestUser(association.id, {
      ...
      const deleted = testDb.prepare('SELECT deleted_at, email FROM users WHERE id = ?').get(user.id) as any;
      expect(deleted.deleted_at).not.toBeNull();
      expect(deleted.email).not.toBe('restoreme@test.com');
      ...
      expect(restoreResponse.body.email).toBe('restoreme@test.com');
      expect(restoreResponse.body.emailRestored).toBe(true);

      const restored = testDb
        .prepare('SELECT deleted_at, status, email, email_before_delete FROM users WHERE id = ?')
        .get(user.id) as any;
      expect(restored.deleted_at).toBeNull();
      expect(restored.status).toBe('active');
      expect(restored.email).toBe('restoreme@test.com');
      expect(restored.email_before_delete).toBeNull();

      // Restored user shows up in the user list again
      const listResponse = await request(app).get('/api/users').set('Authorization', `Bearer ${adminToken}`);
      expect(listResponse.body.data.some((u: any) => u.id === user.id)).toBe(true);
    });

    it('should keep mutated email on restore when original email is taken', async () => {
      const user = createTestUser(association.id, {
        email: 'conflict@test.com',
      });

      await request(app).delete(`/api/users/${user.id}`).set('Authorization', `Bearer ${adminToken}`);

      // Someone else claims the email address in the meantime
      createTestUser(association.id, { email: 'conflict@test.com' });
      ...
      expect(restoreResponse.status).toBe(200);
      expect(restoreResponse.body.emailRestored).toBe(false);
      expect(restoreResponse.body.email).toMatch(/^deleted-\d+-conflict@test\.com$/);

      const restored = testDb.prepare('SELECT deleted_at, email FROM users WHERE id = ?').get(user.id) as any;
      expect(restored.deleted_at).toBeNull();
      expect(restored.email).not.toBe('conflict@test.com');
    });

    it('should return 404 when restoring a user that is not deleted', async () => {
      const response = await request(app)
        .post(`/api/users/${memberUser.id}/restore`)
        .set('Authorization', `Bearer ${adminToken}`);

      expect(response.status).toBe(404);
    });

    it('should deny restore to non-admin users', async () => {
      const response = await request(app)
        .post(`/api/users/${memberUser.id}/restore`)
        .set('Authorization', `Bearer ${memberToken}`);
  ...
  describe('Soft-deleted user authentication', () => {
    it('should not allow a soft-deleted user to login', async () => {
      const user = createTestUser(association.id, {
        email: 'nologin@test.com',
        password: 'supersecret123',
      });

      // Sanity check: login works before deletion
      ...
      // Login with the original email fails
      const loginAfter = await request(app)
        .post('/api/auth/login')
        .send({ email: user.email, password: user.password });
      expect(loginAfter.status).toBe(401);

      // Login with the mutated email also fails (deleted_at filter)
      const mutated = testDb.prepare('SELECT email FROM users WHERE id = ?').get(user.id) as any;
      const loginMutated = await request(app)
        .post('/api/auth/login')
        .send({ email: mutated.email, password: user.password });
      expect(loginMutated.status).toBe(401);
    });
  });

  describe('GET /api/users/directory', () => {
    it('should return member directory for authenticated users', async () => {
      const response = await request(app).get('/api/users/directory').set('Authorization', `Bearer ${memberToken}`);
    ...
    it('should require authentication', async () => {
      const response = await request(app).get('/api/users/directory');

      expect(response.status).toBe(401);
    });

    it('should only return users from the same association', async () => {
      testDb.prepare('INSERT INTO associations (id, name) VALUES (?, ?)').run('other-assoc-5', 'Other Association 5');
