...
import { createTestEnvironment, createTestInstrument } from '../testUtils';
...
describe('Instruments Routes', () => {
  ...
  describe('GET /api/instruments', () => {
    it('should return all instruments', async () => {
      ...
      const response = await request(app).get('/api/instruments').set('Authorization', `Bearer ${memberToken}`);
  ...
  describe('POST /api/instruments', () => {
    it('should create a new instrument as admin', async () => {
      const response = await request(app).post('/api/instruments').set('Authorization', `Bearer ${adminToken}`).send({
        name: 'Clarinet',
        tuning: 'Bb',
        clef: 'sol',
      });
    ...
    it('should reject creation by regular member', async () => {
      const response = await request(app).post('/api/instruments').set('Authorization', `Bearer ${memberToken}`).send({
        name: 'Oboe',
        clef: 'sol',
      });
    ...
    it('should default clef to "sol" when not provided', async () => {
      ...
      const listResponse = await request(app).get('/api/instruments').set('Authorization', `Bearer ${memberToken}`);
