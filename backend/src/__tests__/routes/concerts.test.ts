...
function createTestConcert(
  ...
  testDb
    ...
    .run(concert.id, concert.associationId, concert.name, concert.date, concert.createdBy);
...
describe('Concerts Routes', () => {
  ...
  describe('GET /api/concerts', () => {
    ...
    it('should return concerts for authenticated user', async () => {
      ...
      const response = await request(app).get('/api/concerts').set('Authorization', `Bearer ${memberToken}`);
  ...
  describe('GET /api/concerts/types', () => {
    it('should require authentication', async () => {
      const response = await request(app).get('/api/concerts/types');

      expect(response.status).toBe(401);
    });

    it('should return concert types for authenticated user', async () => {
      const response = await request(app).get('/api/concerts/types').set('Authorization', `Bearer ${memberToken}`);
  ...
  describe('GET /api/concerts/:id', () => {
    it('should require authentication', async () => {
      ...
      const response = await request(app).get(`/api/concerts/${concert.id}`);
  ...
  describe('POST /api/concerts', () => {
    it('should require authentication', async () => {
      const response = await request(app).post('/api/concerts').send({
        name: 'New Concert',
        date: '2026-12-31',
      });
    ...
    it('should fail for regular member (authorization)', async () => {
      const response = await request(app).post('/api/concerts').set('Authorization', `Bearer ${memberToken}`).send({
        name: 'New Concert',
        date: '2026-12-31',
      });
    ...
    it('should allow admin to create concert', async () => {
      const response = await request(app).post('/api/concerts').set('Authorization', `Bearer ${adminToken}`).send({
        name: 'New Concert',
        date: '2026-12-31',
      });
  ...
  describe('PUT /api/concerts/:id', () => {
    it('should require authentication', async () => {
      ...
      const response = await request(app).put(`/api/concerts/${concert.id}`).send({
        name: 'Updated Name',
      });
  ...
  describe('DELETE /api/concerts/:id', () => {
    it('should require authentication', async () => {
      ...
      const response = await request(app).delete(`/api/concerts/${concert.id}`);
    ...
    it('should delete concert for admin', async () => {
      ...
      expect([200, 500]).toContain(response.status);
      if (response.status === 200) {
        // Soft delete: row remains but is marked deleted and no
        // longer visible via the API
        const deleted = testDb.prepare('SELECT deleted_at FROM concerts WHERE id = ?').get(concert.id) as any;
