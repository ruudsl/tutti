...
function importMusicXMLFile(
    ...
    const title = db.prepare('SELECT composer, arranger FROM music_titles WHERE id = ?').get(titleId) as { composer: string | null; arranger: string | null };
...
function createNewTitle(
    ...
    if (!dryRun) {
        ...
        `).run(
            titleId,
            title,
            extractComposer(parsed),
            extractArranger(parsed),
            associationId
        );
    }
...
async function main() {
    ...
    await db.init();
    const association = db.prepare('SELECT id, name FROM associations WHERE id = ?').get(options.associationId) as { id: string; name: string } | undefined;
