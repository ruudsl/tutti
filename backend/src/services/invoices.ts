...
export function generateInvoiceNumber(associationId: string): string {
    ...
    for (const invoice of invoices.values()) {
        if (invoice.associationId === associationId &&
            invoice.createdAt >= todayStart &&
            invoice.createdAt <= todayEnd) {
            count++;
        }
    }
...
export async function generateInvoicePDF(invoiceId: string): Promise<Buffer> {
    ...
    // VAT note
    drawText(
        'BTW-tarief: 9% (verlaagd tarief voor culturele evenementen in Nederland)',
        margin,
        y,
        { size: 8, color: gray }
    );
