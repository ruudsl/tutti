...
export async function sendTelegramNotification(
    ...
    if (data?.url) {
        replyMarkup = {
            inline_keyboard: [[
                { text: 'Openen in app', url: data.url }
            ]]
        };
    }
...
/status - Toon je huidige koppeling status`;
    ...
    const user = db.prepare(`SELECT first_name, last_name FROM users WHERE id = ?`).get(linkData.user_id) as { first_name: string; last_name: string } | undefined;
 ...
 */
export async function handleSettingsCommand(chatId: number): Promise<{ text: string; replyMarkup?: TelegramReplyMarkup }> {
    const channel = db.prepare(`
        SELECT user_id FROM user_notification_channels
        WHERE channel_type = 'telegram' AND channel_id = ?
    `).get(chatId.toString()) as { user_id: string } | undefined;
