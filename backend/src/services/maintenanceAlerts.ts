...
export function logMaintenance(
  ...
  const equipment = db.prepare('SELECT maintenance_interval_months FROM equipment WHERE id = ?').get(equipmentId) as
    { maintenance_interval_months: number } | undefined;
...
export function getAssociationMaintenanceCosts(
  ...
  total: number;
  count: number;
  byEquipment: { equipmentId: string; instrumentType: string; total: number; count: number }[];
} {
  let query = `
        SELECT
