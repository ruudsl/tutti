...
async function sendEmailNotification(
    ...
    const user = db.prepare(`SELECT email, first_name FROM users WHERE id = ?`).get(userId) as { email: string; first_name: string } | undefined;
    if (!user) {
        return false;
    }
