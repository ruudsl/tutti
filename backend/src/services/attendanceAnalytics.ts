 ...
 */
function getDateRange(period: 'week' | 'month' | 'quarter' | 'year', offset: number = 0): { startDate: string; endDate: string } {
    const now = new Date();
    let startDate: Date;
    let endDate: Date;
...
export function getAttendanceOverview(
    ...
    const prevStartDate = new Date(new Date(prevEndDate).getTime() - periodLength * 1000 * 60 * 60 * 24).toISOString().split('T')[0];
...
export function getAttendanceTrends(
    ...
    return rows.map(row => ({
        ...
        attendanceRate: row.total_members > 0
            ? Math.round((row.attended / row.total_members) * 100)
            : 0,
    }));
}
 ...
 */
export function getOrchestraAttendance(
    associationId: string,
    dateRange?: DateRange
): OrchestraAttendance[] {
    const now = new Date();
...
export function exportAttendanceToCSV(
    ...
    // Add summary row
    const summaryRow = ['SUMMARY', '', '', '', '', ...members.map(member => {
        let attended = 0;
        let total = 0;
        rehearsals.forEach(rehearsal => {
            const attendanceMap = rehearsalAttendance.get(rehearsal.id)!;
