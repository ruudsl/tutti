 ...
 */
function getHistoricalData(associationId: string, concertType?: string | null, venueType?: string | null): HistoricalConcertData[] {
    let query = `
        SELECT
            c.id,
            c.name,
...
export function predictTicketSales(concertId: string): SalesPrediction {
    ...
    const avgPrice = totalCapacity > 0 ? ticketTypes.reduce((sum, tt) => sum + (tt.price * tt.quantity), 0) / totalCapacity : 0;
    ...
    if (historicalData.length < 3) {
        ...
        if (historicalData.length === 0) {
            ...
            factors.push(`Using ${historicalData.length} similar concerts and ${allHistoricalData.length - historicalData.length} other concerts for prediction`);
        }
    } else {
...
export function calculateOptimalPricing(concertId: string): PricingSuggestion {
    ...
    const avgCurrentPrice = totalCapacity > 0
        ? ticketTypes.reduce((sum, tt) => sum + (tt.price * tt.quantity), 0) / totalCapacity
        : 0;
