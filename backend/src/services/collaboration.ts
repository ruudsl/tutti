 ...
 */
export function updateSharedLibrary(id: string, name: string, description: string | null, isPublic: boolean): void {
  db.prepare(
    `
...
export function removeTitleFromLibrary(libraryId: string, titleId: string): void {
  db.prepare('DELETE FROM shared_library_titles WHERE library_id = ? AND title_id = ?').run(libraryId, titleId);
...
export function grantLibraryAccess(
  ...
  db.prepare('DELETE FROM library_access WHERE library_id = ? AND association_id = ?').run(libraryId, associationId);
...
export function checkLibraryAccess(
  ...
  // Check if owner
  const library = db.prepare('SELECT owner_association_id FROM shared_libraries WHERE id = ?').get(libraryId) as
    { owner_association_id: string } | undefined;
  ...
  // Check access grant
  const access = db
    .prepare('SELECT access_level FROM library_access WHERE library_id = ? AND association_id = ?')
...
export function respondToRequest(
  ...
  if (status === 'accepted' && request.libraryId) {
    grantLibraryAccess(request.libraryId, request.fromAssociationId, accessLevel || 'view', respondedBy);
  }
...
export function importTitle(
  ...
  db.prepare(
    `
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    `,
  ).run(importId, newTitleId, sourceTitleId, sourceLibraryId, sourceTitle.source_association_id, importedBy);
