...
- Login limiet: ${config.authRateLimitMaxRequests} pogingen per 15 minuten
        ...
        tags: [
            ...
            { name: 'Polls', description: 'Peilingen en stemmingen' },
            { name: 'Tasks', description: 'Takenbeheer' },
            { name: 'Posts', description: 'Nieuwsberichten' },
            { name: 'Email Campaigns', description: 'E-mailcampagnes en templates' },
            { name: 'Contacts', description: 'Externe contacten' },
            { name: 'Custom Fields', description: 'Aangepaste velden' },
            { name: 'Privacy', description: 'Privacy-instellingen' },
            { name: 'Accounting', description: 'Boekhouding - Rekeningen, facturen, transacties' },
        ],
        components: {
            securitySchemes: {
                bearerAuth: {
            ...
            schemas: {
                ...
                Poll: {
                    ...
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        title: { type: 'string', example: 'Repetitiedatum extra repetitie' },
                        description: { type: 'string', nullable: true },
                        pollType: { type: 'string', enum: ['single', 'multiple', 'ranked'], example: 'single' },
                        status: { type: 'string', enum: ['draft', 'active', 'closed', 'archived'], example: 'active' },
                        isAnonymous: { type: 'boolean', example: false },
                        showResultsBeforeClose: { type: 'boolean', example: true },
                        ...
                        optionCount: { type: 'integer', example: 4 },
                        hasVoted: { type: 'boolean', example: true },
                        createdBy: { type: 'string', format: 'uuid' },
                        createdByName: { type: 'string', example: 'Jan de Dirigent' },
                        createdAt: { type: 'string', format: 'date-time' },
                    },
                },
                PollOption: {
                    ...
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        label: { type: 'string', example: 'Vrijdag 15 maart' },
                        sortOrder: { type: 'integer', example: 0 },
                        voteCount: { type: 'integer', example: 8 },
                    },
                },
                CreatePollRequest: {
                    type: 'object',
                    required: ['title', 'pollType', 'options'],
                    properties: {
                        title: { type: 'string' },
                        description: { type: 'string' },
                        pollType: { type: 'string', enum: ['single', 'multiple', 'ranked'] },
                        isAnonymous: { type: 'boolean', default: false },
                        showResultsBeforeClose: { type: 'boolean', default: true },
                ...
                Task: {
                    ...
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        title: { type: 'string', example: 'Vleugel stemmen voor concert' },
                        description: { type: 'string', nullable: true },
                ...
                TaskList: {
                    ...
                    properties: {
                        ...
                        taskCount: { type: 'integer', example: 8 },
                    },
                },
                CreateTaskRequest: {
                    type: 'object',
                ...
                Post: {
                    ...
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        title: { type: 'string', example: 'Kerstconcert aankondiging' },
                        excerpt: { type: 'string', nullable: true },
                        status: { type: 'string', enum: ['draft', 'published', 'archived'] },
                        isPinned: { type: 'boolean', example: false },
                        isFeatured: { type: 'boolean', example: true },
                        allowComments: { type: 'boolean', example: true },
                        viewCount: { type: 'integer', example: 45 },
                ...
                PostCategory: {
                    ...
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        name: { type: 'string', example: 'Concerten' },
                        slug: { type: 'string', example: 'concerten' },
                ...
                CreatePostRequest: {
                    ...
                    properties: {
                        ...
                        targetOrchestras: { type: 'array', items: { type: 'string', format: 'uuid' } },
                ...
                EmailCampaign: {
                    ...
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        name: { type: 'string', example: 'Nieuwsbrief december' },
                        subject: { type: 'string', example: 'Nieuwsbrief december 2024' },
                        status: { type: 'string', enum: ['draft', 'scheduled', 'sending', 'sent', 'cancelled'] },
                        targetType: { type: 'string', enum: ['all', 'orchestras', 'roles', 'custom'] },
                        scheduledAt: { type: 'string', format: 'date-time', nullable: true },
                        sentAt: { type: 'string', format: 'date-time', nullable: true },
                        totalRecipients: { type: 'integer', example: 75 },
                        deliveredCount: { type: 'integer', example: 73 },
                        openedCount: { type: 'integer', example: 45 },
                        clickedCount: { type: 'integer', example: 12 },
                        bouncedCount: { type: 'integer', example: 2 },
                        createdBy: { type: 'string', format: 'uuid' },
                        createdByName: { type: 'string' },
                        createdAt: { type: 'string', format: 'date-time' },
                    },
                ...
                EmailTemplate: {
                    ...
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        name: { type: 'string', example: 'Standaard nieuwsbrief' },
                        subject: { type: 'string', example: 'Nieuwsbrief {maand}' },
                ...
                CreateEmailCampaignRequest: {
                    type: 'object',
                    required: ['name', 'subject', 'bodyHtml', 'targetType'],
                    properties: {
                        name: { type: 'string' },
                        subject: { type: 'string' },
                        bodyHtml: { type: 'string' },
                ...
                Contact: {
                    ...
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        contactType: { type: 'string', enum: ['organization', 'person', 'venue', 'vendor'] },
                        name: { type: 'string', example: 'Gemeente Eindhoven Cultuurfonds' },
                ...
                CustomFieldDefinition: {
                    ...
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        entityType: { type: 'string', enum: ['user', 'orchestra', 'rehearsal', 'concert', 'music_piece', 'loan', 'instrument', 'contact'] },
                        fieldKey: { type: 'string', example: 'tshirt_size' },
                        fieldLabel: { type: 'string', example: 'T-shirt maat' },
                        fieldType: { type: 'string', enum: ['text', 'textarea', 'number', 'date', 'datetime', 'boolean', 'select', 'multiselect', 'email', 'phone', 'url', 'file'] },
                        fieldOptions: { type: 'string', nullable: true, description: 'JSON array voor select/multiselect' },
                        isRequired: { type: 'boolean', example: false },
                        isUnique: { type: 'boolean', example: false },
                        selfEditable: { type: 'boolean', example: true },
                        sortOrder: { type: 'integer', example: 0 },
                        description: { type: 'string', nullable: true },
                ...
                PrivacySettings: {
                    type: 'object',
                    properties: {
                        fieldName: { type: 'string', example: 'phone' },
                        visibility: { type: 'string', enum: ['admin_only', 'committee', 'orchestra', 'section', 'all_members', 'public'] },
                    },
                },
            },
            responses: {
